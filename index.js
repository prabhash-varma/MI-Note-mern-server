...
mongoose.connect("mongodb+srv://prabhash:prabhash@cluster0.u7qtlq4.mongodb.net/?retryWrites=true&w=majority", {
...
app.post("/additem", verifyJWT, async (req, res) => {
  ...
  let p = await Items.find({title:title,email:email });

  if (p.length > 0) {
    res.json({ auth: false, message: "Item already exists" });
  }
  else {
    ...
    await item.save().then(() => {
      ...
      console.log(err);
      res.json({ auth: false, message: "Item not added" });
    });
